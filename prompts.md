...
**Update (post-audit — §3.5 parity):** added 5 unit tests in `tickets.service.spec.ts` covering soft-delete (`remove` uses `softRemove` not hard delete, `findDeleted` filter shape pinned to `withDeleted: true` + `deletedAt: Not(IsNull())`, `restore` happy path + both NotFound branches for not-deleted and missing rows) and one e2e (`test/soft-delete.e2e-spec.ts`) that walks the full ticket lifecycle and asserts soft-deleted rows are hidden from standard `GET /tickets?projectId=` responses — the headline §3.5 claim. `ProjectsService` soft-delete uses the identical pattern; not separately unit-tested to avoid duplicating coverage that the ticket spec already pins.